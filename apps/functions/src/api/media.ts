...
/**
 * The full resolve → validate → range → stream pipeline, shared by both
 * `/api/media/:assetId` (owner-session-protected, any registered asset) and
 * `/api/public-media/:assetId` (public, allowlist-gated — see
 * `api/public-media.ts`). Access control is entirely the CALLER's
 * responsibility (session middleware for the former, the explicit
 * allowlist check for the latter) — this function itself has no concept of
 * who's allowed to ask for `assetId`, only how to safely resolve and serve
 * whatever it's given once that's already been decided. Every request is
 * resolved through the accepted Sheet gateway first (`resolveMediaAsset`);
 * a Drive call only ever happens for an enabled asset at the exact
 * requested version. Never accepts a raw Drive file ID from the browser,
 * and never returns one, an authorization header, an access token,
 * service-account data, or a raw Google error in any response.
 */
export async function serveMediaAsset(
  ...
  if (typeof variant === 'object') {
    sendError(res, new AppError('MEDIA_ASSET_INVALID', 'The requested variant is not recognized.'));
    return;
  }
  ...
  try {
    ...
    if (metadata.trashed) {
      throw new AppError('MEDIA_FILE_INACCESSIBLE', 'The referenced media file is not accessible.');
    }
    ...
    if (metadata.size === null) {
      throw new AppError('MEDIA_FILE_INACCESSIBLE', 'The referenced media file has no known size.');
    }
 ...
 * — owner-session-protected (wired in app.ts): any registered/enabled
 * asset, not just an allowlisted few.
 */
